...
import type { Settings, Entry, DayRow, SaldoModo } from "@/lib/types"
...
function buildRows(settings: Settings, entries: Entry[]): DayRow[] {
  ...
  const base = settings.saldo_modo === 'projetado'
    ? settings.saldo_projetado
    : settings.saldo_inicial
  let running = base
...
export default function HomePage() {
  ...
  async function handleSaldoChange(value: number) {
    if (!settings) return
    const field = settings.saldo_modo === 'projetado' ? 'saldo_projetado' : 'saldo_inicial'
    const { error } = await sb()
      .from("settings")
      .update({ [field]: value })
      .eq("id", 1)
    if (error) toast.error("Erro ao salvar saldo")
    else setSettings((prev) => prev ? { ...prev, [field]: value } : prev)
  }

  async function handleModoChange(modo: SaldoModo) {
    const { error } = await sb()
      .from("settings")
      .update({ saldo_modo: modo })
      .eq("id", 1)
    if (error) toast.error("Erro ao alterar modo")
    else setSettings((prev) => prev ? { ...prev, saldo_modo: modo } : prev)
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="fixed bottom-16 left-0 right-0 z-10 bg-white border-t border-gray-100">
        <RangeManager
          settings={settings}
